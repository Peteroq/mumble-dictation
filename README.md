...
## Not the other Mumble

There is a separate, pre-existing app at `~/coding/whisper-flow-clone_2.20.26/Mumble.app`.
...
| | Existing Mumble | This project |
|---|---|---|
| Bundle ID | `com.mumble.client` | `ai.pivotstudio.mumble` |
| Executable | `mumble` | `Mumble` |
| Location | `~/coding/whisper-flow-clone_2.20.26/` | `~/Desktop/mumble-build/` |
...
in more than case, so `pkill -x Mumble` can never match `mumble`. The `Makefile`
only ever targets `$(EXEC)`, never a bare `Mumble`.
